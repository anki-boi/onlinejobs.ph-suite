...
(live FX rates from Frankfurter/ECB — no default/fallback rate: a currency
with no live rate stays NULL), plus `norm_title` and `deleted_at` (the §4.3
v5 columns W4.5/W4.6 build on). Backfills existing salary text, keeps the
raw string and the v4 raw min/max, and records the rate used in
`app_settings.fx_metadata` (quality bar: rate + timestamp stored together
so the UI can say what rate it used).
...
def step(conn) -> None:
    ...
    if rows:
        # One live rate fetch for the whole backfill (no fallback rate: a
        # currency with no live rate simply stays NULL).
        parsed = []
        curs = set()
        for r in rows:
        ...
        if n and fx:  # metadata records live rates — PHP-only rows need none
            meta = {str(k).lower(): v for k, v in fx.items() if k != "PHP"}
            meta["at"] = _now()
            conn.execute(
